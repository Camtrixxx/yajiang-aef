...
def continuous_recon_loss(pred: torch.Tensor, target: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
...
def batch_variance_loss(emb: torch.Tensor, target_std: float = 1.0, eps: float = 1e-4) -> torch.Tensor:
    if emb.shape[0] <= 1:
        return emb.new_tensor(0.0)
...
def compute_total_loss(model_output, batch: dict, cfg) -> LossOutput:
    ...
    for tgt in cfg.data.target_sources:
        ...
        if loss_type == "categorical":
            cur = categorical_recon_loss(pred, target, mask=mask)
        else:
